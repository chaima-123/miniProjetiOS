...
class ChatVC: UIViewController {
    ...
    private func fetchChatData() {
        ...
        Service.shared.showChat(user1: userName!.firstName , user2: name!){ (res) in
                self.chatCollView.refreshControl?.endRefreshing()
                switch res {
                case .failure(let err):
                    print("Failed to fetch chat:", err)
                case .success(let mang):
    //                print(posts)
                    self.chatsArray = mang
    ...
    @IBAction func onSendChat(_ sender: UIButton?) {
        ...
        chatTF.text = ""
        let chat = Chat.init(user_name: name, user_image_url: "https://", is_sent_by_me: false, text: chatText)
        Service.shared.addChat(user1: name! , user2:userName!.firstName, msg: chatText, isSent: false) { (res) in
            switch res {
            case .failure(let err):
